...
if ($DTE.Version -eq '12.0' -or $DTE.Version -eq '14.0')
{
    ...
    if ($installRoslynPackage -eq $false)
    {
        $dllGacPath = Join-Path $gacPath 'Microsoft.CodeAnalysis.Desktop'
        $dllName = 'Microsoft.CodeAnalysis.Desktop.dll'
        $dllVersion = '1.0.0.50108'
        $install = $true
        if (Test-Path $dllGacPath)
        {
    ...
    if ($installRoslynPackage -eq $false)
    {
        $dllGacPath = Join-Path $gacPath 'Microsoft.CodeAnalysis.CSharp.Desktop'
        $dllName = 'Microsoft.CodeAnalysis.CSharp.Desktop.dll'
        $dllVersion = '1.0.0.50108'
        $install = $true
        if (Test-Path $dllGacPath)
        {
    ...
    if ($installRoslynPackage -eq $false)
    {
        $dllGacPath = Join-Path $gacPath 'Microsoft.CodeAnalysis.CSharp.Workspaces.Desktop'
        $dllName = 'Microsoft.CodeAnalysis.CSharp.Workspaces.Desktop.dll'
        $dllVersion = '1.0.0.50108'
        $install = $true
        if (Test-Path $dllGacPath)
        {
    ...
    if ($installRoslynPackage -eq $true)
    {
        Install-Package Microsoft.CodeAnalysis -Pre -Version 1.0.0-beta2
        cd $gacUtilFolder
        $packagesPath = Join-Path $toolsPath "..\.."
        foreach ($roslynPackagePath in [System.IO.Directory]::GetDirectories($packagesPath)|?{[System.IO.Path]::GetFileName($_).EndsWith("Microsoft.CodeAnalysis.") -or [System.IO.Path]::GetFileName($_).StartsWith("System.Collections.Immutable.")  -or [System.IO.Path]::GetFileName($_).StartsWith("System.Reflection.Metadata.")})
        {
        	$roslynPackageLibPath = Join-Path $roslynPackagePath "lib\net45"
